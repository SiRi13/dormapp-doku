...
Dazu wurden zwei einfache Tabellen angelegt. Die Tabelle \verb!tp_test! in Abb. \ref{sql:tptest} wird für die Schreib- und Lesevorgänge
verwendet. Um alle nötigen Datentypen testen zu können, wurden verschiedene Spalten verwendet. 
Dadurch konnte der spätere Einsatz besser simuliert werden.

\begin{lstlisting}[float=h, caption=Aufbau der Tabelle tp\_test, captionpos=below, label=sql:tptest, language=sql, escapechar=|]
...
\begin{lstlisting}[float=h, caption=Aufbau der Tabelle users, captionpos=below, label=sql:usersTest, language=sql, escapechar=|]
...
Wie in Abbildung \ref{dirtree:phpPrototyp} \nameref{dirtree:phpPrototyp} zu sehen, wurde der Aufbau zunächst in zwei Kategorien aufgeteilt.
...
Die vollständigen Skripte befinden sich im Anhang und werden hier nur in Auszügen dargestellt. \\
...
Sind alle Parameter korrekt übertragen worden, wird die Funktion \verb!getUserByEmailAndPassword($email, $password)!, die sich im Skript \verb!DB\_Functions.php! befindet, aufgerufen. Zum Authentifizieren wird aus der Datenbank der zur
...
\begin{lstlisting}[float=h, caption=Auszug aus login.php, captionpos=below, label=php:loginProto, language=php, escapechar=|]
<?php
[|\ldots{}|]
if (isset($tag) && $tag != `') {
...
\begin{lstlisting}[float=h, caption=Auszug aus DB\_Functions.php, captionpos=below, label=php:dbFuncProto, language=php, escapechar=|]
[|\ldots{}|]
private $db;
...
näher erläutert. \\
Der Ordnerbaum in Abb. \ref{dirtree:dbProto} zeigt die wichtigen Ordner und Dateien, und erläutert kurz deren Funktion.
...
\begin{lstlisting}[float=h, caption=Auszug aus menu/main.xml, captionpos=below, label=xml:mainMenuProto, language=xml, escapechar=|]
[|\ldots{}|] 
...
\begin{lstlisting}[float=h, caption=Auszug aus MainActivity.java, captionpos=below, label=java:MainActivityProto, language=java, escapechar=|]
[|\ldots{}|]
@Override
...
Für die Funktion des Prototypen und das Umsetzen der definierten Schwerpunkte sind in diesem Projekt die beiden Klassen
\verb!LoginActivity.java! und \verb!MainActivity.java! von Bedeutung.
Beim Start der App wird zunächst die \verb!MainActivity.java! mit dem Layout \verb!activity_main.xml! geladen (siehe Abb. \ref{pic:MainActivityProto}). \\* Das geschieht in Zeile \ref{line:setContentViewProto} durch
...
\verb!SQLite!-Datenbank schreibt. Diese Klasse wird in dem Kapitel \ref{subsub:DBConLibProto} \verb!DBConnectionLib! genauer erläutert.

\begin{lstlisting}[float=h, caption=Auszug aus der MainActivity.java, captionpos=below, label=java:MainActivityProto, language=java, escapechar=|]
[|\ldots{}|]
@Override
...
Wurden die Tabellen erfolgreich synchronisiert, können über die beiden Felder Testwerte eingegeben werden. Durch einen
Klick auf den Button \texttt{Save to DB} werden die Daten zunächst in der Methode \verb!saveToDb(View view)!(Zeile ~\ref{line:saveToDbProto})
...
\begin{lstlisting}[caption=Auszug aus der MainActivity.java, captionpos=below, label=java:MainActivityProto3, language=java, escapechar=|]
[|\ldots{}|]        
...
an die Methode \textsl{onPostExecute(JSONObject json)}(Zeile \ref{line:postExecProto}) übergeben.
...
Die so verpackten Parameter werden schließlich an den \verb!JSONParser! (siehe Listing \ref{java:JSONParserProto}) übergeben.
...
Die so verpackte Tabelle, wird dann mit einer \verb!for!-Schleife (Zeile \ref{line:parseJsonProto} in die einzelnen Zeilen zerlegt und von der Funktion
\textsl{addTestRow(\ldots{})} (Zeile \ref{line:addTstRw()Proto}) in die lokale \verb!SQLite!-Datenbank geschrieben
...
gekapselt und dem \verb!JSONParser! (siehe Listing \ref{java:JSONParserProto}), zum Abschicken der Anfrage übergeben.
